...
let match_result =
  Re.compile Re.(alt [str "sat"; str "unsat"; str "unknown"])
...
let check_alt_ergo query =
  ...
  let result = Option.map (fun g -> Re.Group.get g 0 |> result_of_string) (Re.exec_opt match_result output) in
