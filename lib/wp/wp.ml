...
type kind = Pre_implies_wp | Loop_invariant_preserved | Loop_exit | Assertion

let pp_kind ppf kind =
  match kind with
  | Pre_implies_wp -> Fmt.string ppf "entry implies wp"
  | Loop_invariant_preserved -> Fmt.string ppf "loop invariant preserved"
  | Loop_exit -> Fmt.string ppf "loop exit"
...
let rec vcs (cmd : Cmd.t) post =
  ...
  | Seq (left, right) ->
      ...
      let vcs_right =
        List.map vcs_right ~f:(fun vc -> { vc with goal = wp left vc.goal })
      in
      List.concat [ vcs_left; vcs_right ]
  | If (cond, then_, else_) ->
      let cond_form = Conv.to_formula cond in
      let vcs_then =
        List.map (vcs then_ post) ~f:(fun vc ->
            { vc with goal = Imp (cond_form, vc.goal) })
      in
      let vcs_else =
        List.map (vcs else_ post) ~f:(fun vc ->
            { vc with goal = Imp (Not cond_form, vc.goal) })
      in
  ...
  | While (cond, inv, body) ->
      ...
      let cond_form = Conv.to_formula cond in
      let body_vcs =
        List.map (vcs body inv) ~f:(fun vc ->
            { vc with goal = Imp (And (inv, cond_form), vc.goal) })
      in
      List.concat
        [
          body_vcs;
          [
            {
              loc;
              kind = Loop_invariant_preserved;
              goal = Formula.(Imp (And (inv, cond_form), wp body inv));
            };
          ];
          [
            {
              loc;
              kind = Loop_exit;
              goal = Formula.(Imp (And (inv, Not cond_form), post));
            };
          ];
        ]
...
let verify (prog : Program.t) =
  List.concat
    [
      ...
      |> List.map ~f:(fun vc -> { vc with goal = Imp (prog.pre, vc.goal) });
    ]

let verify_and_check (prog : Program.t) =
  let vcs = verify prog in
  vcs |> List.map ~f:(fun vc -> (vc, Smt.Solver.check_alt_ergo vc.goal))
