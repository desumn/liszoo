...
let rec equal (cmd1 : t) (cmd2 : t) =
  ...
  | While (cond1, inv1, body1), While (cond2, inv2, body2) ->
      Bool_expr.equal cond1 cond2
      && Formula.equal inv1 inv2 && equal body1 body2
  | (Skip | Assign _ | Seq _ | If _ | While _ | Assert _ | Assume _), _ -> false
...
and pp_node ppf (cmd : t) =
  ...
  | Seq (s1, s2) -> Fmt.pf ppf "%a;@,%a" pp_node s1 pp_node s2
  | If (c, t, e) ->
      Fmt.pf ppf "@[<v 2>if %a then@,%a@]@,@[<v 2>else@,%a@]@,end" Bool_expr.pp
        c pp_node t pp_node e
  | While (c, i, b) ->
      Fmt.pf ppf "@[<v 2>while %a invariant { %a } do@,%a@]@,end" Bool_expr.pp c
        Formula.pp i pp_node b
