...
let verdict_line ~num ~total (vc : Wp.vc) (verdict : Smt.Solver.smt_result) =
  let open Notty in
  let open Nottui in
  let icon, colour = match verdict with
    | Valid   -> "✓", A.green
    | Invalid -> "✗", A.red
    | Unknown _ -> "?", A.yellow
  in
...
let parse_and_verify file =
  ...
  | Ok (program, _content) ->
    ...
    let verdict_uis = List.mapi verdicts
      ~f:(fun i (vc, verdict) ->
        match verdict with
        | Ok verdict -> verdict_line ~num:(i + 1) ~total:num_verdicts vc verdict
        | Error (`Msg message)->
...
let cmd =
  ...
  Cmd.v info
    Term.(const parse_and_verify $ program_arg)
